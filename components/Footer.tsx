...
// Apple-style footer: fine print → link directory → legal bar.
export function Footer() {
  return (
    <footer
      className="w-full border-t px-6 py-[48px]"
      style={{ borderColor: 'var(--hairline)', background: 'var(--void)' }}
      role="contentinfo"
    >
      <div className="mx-auto flex flex-col gap-[32px]" style={{ maxWidth: 980 }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-[28px] pb-[24px]" style={{ borderBottom: '1px solid var(--hairline)' }}>
          {COLUMNS.map(col => (
            <nav key={col.title} className="flex flex-col gap-[10px]" aria-label={col.title}>
              <div className="font-mono lowercase" style={{ fontSize: 11, letterSpacing: '.14em', color: 'var(--ghost2)' }}>
                {col.title}
              </div>
              {col.links.map(link => (
                <a
